...
func Test_Auth(t *testing.T) {
	...
	err = sign(r)
	assert.NoError(t, err)
	assert.Equal(t, r.Header.Get(auth.HeaderServiceAk), ak)
	assert.Equal(t, r.Header.Get(auth.HeaderServiceShaAKSK), shaaksk)
	assert.Equal(t, r.Header.Get(auth.HeaderServiceProject), project)
	assert.NotEmpty(t, r.Header.Get(hws_cloud.HeaderAuthorization))

	sign, err = auth.GetShaAKSKSignFunc(ak, sk, project)
	r.Header = nil
	assert.NoError(t, err)
	r, err = http.NewRequest("GET", "http://127.0.0.1:8080", nil)
	assert.NoError(t, err)
